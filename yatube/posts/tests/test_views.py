...
class CommentViewsTest(TestCase):
    ...
    #def test_new_comment_appears_on_page_post_detail(self):
    #    """После успешной отправки комментарий появляется на странице поста."""
    #    form_data = {'text': 'Тестовый комментарий к посту', }
    #    response = self.guest_client.post(
    #        reverse('posts:add_comment', kwargs={'post_id': self.post.id, }),
    #        data=form_data,
    #        follow=True,
    #    )
    #    expected_context = self.post.comments.last()
    #    url = [
    #        reverse('posts:post_detail', kwargs={'post_id': self.post.id, }),
    #    ]
    #    response = self.authorized_client.get(url)
    #    self.assertEqual(len(response.context['comments']), 1)
    #    current_context = response.context['comments'][0]
    #    self.assertEqual(current_context, expected_context)
